...
def plot_drone_tet_fo_eval(time, rpy_max, l_tet_min,  simU, simX, latexify=False):
    if latexify:
        latexify_plot()
    ...
    # Plot tether length command
    axs[2].plot(time[:-1], simU[:, 4], label=r'$l_{\mathrm{tether_{cmd}}}$')
    axs[2].axhline(l_tet_min, color='gray', linestyle='--', label='')
    axs[2].set_ylabel("Tether Cmd [m]")
    axs[2].legend()
    axs[2].grid(True)

    # Plot tether length
    axs[3].plot(time, simX[:, -1], label=r'$l_{\mathrm{tether}}$')
    axs[3].set_ylabel("Tether Length [m]")
    axs[3].legend()
    axs[3].grid(True)

    # Plot drone position
    axs[4].plot(time, simX[:, 0], label='x')
